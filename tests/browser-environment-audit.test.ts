...
describe("browser environment consistency audit", () => {
  test("accepts a coherent Windows/Win32/ANGLE hardware profile", () => {
    ...
    expect(audit.userAgentOs).toBe("windows");
    expect(audit.platformOs).toBe("windows");
  ...
  test("warns when a Windows user agent reports Linux platform and Mesa software rendering", () => {
    const audit = classifyBrowserEnvironment(snapshot({
      ...
      webglRenderer: "llvmpipe (LLVM 18.1.8, 256 bits)"
    }));
  ...
  test("warns for SwiftShader even when user agent and platform match", () => {
    ...
    expect(audit.status).toBe("warning");
    expect(audit.issues.map(issue => issue.code)).toContain("software-renderer");
    expect(audit.issues.map(issue => issue.code)).not.toContain("ua-platform-mismatch");
  });
  ...
  test("does not treat ordinary Mesa hardware rendering on Linux as a mismatch", () => {
    const audit = classifyBrowserEnvironment(snapshot({
      ...
      platform: "Linux x86_64",
      webglVendor: "Intel",
      webglRenderer: "Mesa Intel(R) UHD Graphics 770 (RPL-S)"
    }));

    expect(audit.status).toBe("green");
  });

  test("wires the audit into launcher, Early Gate runtime and Drop Setup status", () => {
    ...
    expect(launcher).toContain("collectBrowserEnvironment(page)");
    expect(earlyGate).toContain("browserEnvironment: handle.environmentAudit");
    expect(dropUi).toContain("Env GRÜN");
    expect(dropUi).toContain("Env WARNUNG");
  });
});
