...
describe("profile cookie snapshot architecture guard", () => {
  ...
  const browserWorker = read("src/browser-worker/seleniumbase-browser-worker.ts");
  ...
  it("injects the explicitly selected snapshot before publishing the task context", () => {
    const addCookiesAt = browserWorker.indexOf("await context.addCookies(snapshot as unknown[])");
    const publishAt = browserWorker.indexOf("this.sessions.set(config.taskId");
    expect(addCookiesAt).toBeGreaterThan(-1);
    expect(publishAt).toBeGreaterThan(addCookiesAt);
  });
