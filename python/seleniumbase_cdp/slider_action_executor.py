...
class SliderActionExecutor:
    ...
    def apply(self, target_fraction: float = 0.96) -> Dict[str, Any]:
        ...
        if state.get("kind") != "slider":
            return {"moved": False, "targetFraction": 0.0, "state": state}

        target = max(0.0, min(1.0, float(target_fraction)))
        moved = self._apply_document(target)
        return {"moved": moved, "targetFraction": target, "state": self._slider_adapter.poll()}
    ...
    def _apply_document(self, target: float) -> bool:
        ...
        (() => {{
          ...
          for (const root of roots) {{
            let handles = overrides.sliderHandle ? [...root.querySelectorAll(overrides.sliderHandle)].filter(visible) : [];
            if (!handles.length) handles = [...root.querySelectorAll('input[type="range"],[role="slider"],[aria-valuenow]')].filter(visible);
            if (!handles.length) handles = [...root.querySelectorAll('[class*="slider" i] [class*="thumb" i],[class*="slider" i] [class*="handle" i],[class*="drag" i] [class*="handle" i]')].filter(visible);
            for (const handle of handles) {{
              ...
              if (handle.matches('input[type="range"]')) {{
