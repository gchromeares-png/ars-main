...
/**
 * Generic checkout payment preparation for browser-driven commerce flows.
 * It may select/fill visible payment controls, but it never performs external
 * authorization and never submits the order.
 */
export class CheckoutPaymentPreparer {
  async prepare(page: Page, session?: CheckoutPaymentSession): Promise<PaymentPreparationResult> {
    ...
    result.note = result.missingFields.length
      ...
      : "Kartendaten vorbereitet. Der finale Bestell-Submit bleibt separat geschützt.";
    return result;
  }
    ...
    for (const frame of this.frames(page)) {
      for (const selector of selectors) {
        ...
        try {
          ...
          // Hosted card fields may move between frames while checkout renders.
        }
      }
    }
