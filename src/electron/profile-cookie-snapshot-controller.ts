...
/** Register explicit cookie snapshot operations and return the shared encrypted vault. */
export function registerProfileCookieSnapshotIpc(
  ...
  ipcMain.handle("list-profile-cookie-snapshots", (_event, profileId: string) => {
    try {
      return {
        success: true,
        snapshots: vault.list(profileId),
  ...
  ipcMain.handle("save-profile-cookie-snapshot", async (
    _event,
    profileId: string,
    name: string,
    snapshotId?: string
